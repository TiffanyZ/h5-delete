...
axios.interceptors.request.use((config) => {
  ...
  } else if (config['url'].indexOf('collect') > -1 || config['url'].indexOf('opinion/save') > -1) { // 收藏&反馈
    let currentUrl = window.location.href;
    if (currentUrl.indexOf('myKey') > -1) {
      let token = '';
      if (config['url'].indexOf('collect') > -1) {
        token = currentUrl.split('myKey=')[1].split(' ')[0];
      } else {
        token = currentUrl.split('myKey=')[1].split('&')[0];
      }
      config.headers['access_token'] = token || '';
    } else {
      config.headers['access_token'] = window.LOCAL_CONFIG.ACCESS_TOKEN;
    }
  }
  return config;
}, (error) => {
  ...
  if (method === 'get') {
    ...
    if (data !== undefined) {
      if (prefix === 'ths') {
        ...
        let vValue = new Date().getTime()+''+Math.floor(Math.random()*9999+1000);
        defaultConfig.params = Object.assign(defaultConfig.params, {_v: vValue})
      } else {
